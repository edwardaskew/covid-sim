...
double CalcHouseInf(int j, unsigned short int ts, param const& P, person const* Hosts, household const * Households)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationHouseEffectiveness : 1.0)
		...
		*	P.HouseholdDenomLookup[Households[Hosts[j].hh].nhr - 1] * CalcPersonInf(j, ts, P, Hosts);
}
double CalcPlaceInf(int j, int k, unsigned short int ts, param const& P, person const* Hosts)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationEffectiveness : 1.0)
		...
		*	P.PlaceTypeTrans[k] / P.PlaceTypeGroupSizeParam1[k] * CalcPersonInf(j, ts, P, Hosts);
}
double CalcSpatialInf(int j, unsigned short int ts, param const& P, person const* Hosts)
{
	return	((HOST_ISOLATED(j) && (Hosts[j].digitalContactTraced != 1)) ? P.CaseIsolationEffectiveness : 1.0)
		*	((Hosts[j].digitalContactTraced==1) ? P.DCTCaseIsolationEffectiveness : 1.0)
		...
		*	CalcPersonInf(j, ts, P, Hosts); 		/*	*Hosts[j].spatial_norm */
}
double CalcPersonInf(int j, unsigned short int ts, param const& P, person const* Hosts)
{
...
double CalcPersonSusc(int ai, unsigned short int ts, int infector, int tn, param const& P, person const* Hosts)
{
