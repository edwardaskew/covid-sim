...
void InitModel(int, bitmap_state const* state, param& P, person * Hosts); //adding run number as a parameter for event log: ggilani - 15/10/2014
void SeedInfection(double, int*, int, int, bitmap_state const* state, param& P, person * Hosts); //adding run number as a parameter for event log: ggilani - 15/10/2014
int RunModel(int, bitmap_header const* bmh, unsigned char* bmf, unsigned char* bmPixels, bitmap_state const* state, param& P, person * Hosts); //adding run number as a parameter for event log: ggilani - 15/10/2014
void TravelReturnSweep(double, param const& P, person* Hosts);
void TravelDepartSweep(double, param const& P, person* Hosts);
void InfectSweep(double, int, bitmap_state const* bmh, param const& P, person* Hosts); //added int as argument to InfectSweep to record run number: ggilani - 15/10/14
...
void SaveDistribs(param const& P, person const* Hosts);
...
void LoadSnapshot(param& P, person * Hosts);
void SaveSnapshot(param const& P, person const* Hosts);
void RecordInfTypes(param& P, person* Hosts);
void RecordSample(double, int, bitmap_header const* bmh, unsigned char const* bmf, unsigned char* bmPixels, bitmap_state const* state, param& P, person const* Hosts);
...
#pragma omp parallel default(shared)
	...
	person* Hosts;
	SetupModel(DensityFile, NetworkFile, SchoolFile, RegDemogFile, P, Hosts);
	...
	for (i = 0; (i < P.NumRealisations) && (P.NRactNE < P.NumNonExtinctRealisations) && (!InterruptRun); i++)
	{
		...
		InitModel(i, &state, P, Hosts); //passing run number into RunModel so we can save run number in the infection event log: ggilani - 15/10/2014
		
		if (P.DoLoadSnapshot) LoadSnapshot(P, Hosts);
		while (RunModel(i, bmh, bmf, bmPixels, &state, P, Hosts))
...
void InitModel(int run, bitmap_state const* state, param& P, person * Hosts) // passing run number so we can save run number in the infection event log: ggilani - 15/10/2014
{
...
#pragma omp parallel for private(i,j,k,l,m,tn) reduction(+:nim) schedule(static,1)
	...
	// note that this breaks determinism of runs if executed due to reordering of Cell members list each realisation
						...
						else
						{
							...
							{
								DoImmune(k, state, P, Hosts); nim += 1;
							}
						}
					}
			}
...
#pragma omp parallel for private(m,l) schedule(static,1)
	...
	SeedInfection(0, nsi, 0, run, state, P, Hosts);
...
void SeedInfection(double t, int* nsi, int rf, int run, bitmap_state const* state, param& P, person * Hosts) //adding run number to pass it to event log
{
			...
			for (k = 0; (k < nsi[i]) && (m < 10000); k++)
			{
				...
				if (Hosts[l].inf == InfStat_Susceptible) //// If Host l is uninfected.
				{
					if (CalcPersonSusc(l, 0, 0, 0, P, Hosts) > 0)
					{
						...
						DoInfect(l, t, 0, run, state, P, Hosts); ///// guessing this updates a number of things about person l at time t in thread 0 for this run.
						m = 0;
					}
				}
				...
				for (k = 0; (k < nsi[i]) && (m < 10000); k++)
				{
					...
					if (Hosts[l].inf == InfStat_Susceptible)
					{
						if (CalcPersonSusc(l, 0, 0, 0, P, Hosts) > 0)
						{
							...
							DoInfect(l, t, 0, run, state, P, Hosts);
							m = 0;
						}
					}
					else
...
int RunModel(int run, bitmap_header const* bmh, unsigned char* bmf, unsigned char *bmPixels, bitmap_state const* state, param& P, person * Hosts) //added run number as parameter
{
	...
	{
		RecordSample(t, ns - 1, bmh, bmf, bmPixels, state, P, Hosts);
		...
		if (!InterruptRun)
		{
			...
			for (j = 0; ((j < P.UpdatesPerSample) && (!InterruptRun) && (continueEvents)); j++)
			{
				...
				if (fs)
				{
					if (P.DoAirports) TravelDepartSweep(t, P, Hosts);
					k = (int)t;
					if (P.DurImportTimeProfile > 0)
					{
					...
					if (ir > 0) //// if infection import rate > 0, seed some infections
					{
						...
						if (ni > 0)		SeedInfection(t, nsi, 1, run, state, P, Hosts);
					}
					...
					if (!P.DoSI) IncubRecoverySweep(t, run, state, P, Hosts);
					// If doing new contact tracing, update numbers of people under contact tracing after each time step

					if (P.DoDigitalContactTracing) DigitalContactTracingSweep(t, state, P, Hosts);
				...
				if ((P.DoSaveSnapshot) && (t <= P.SnapshotSaveTime) && (t + P.TimeStep > P.SnapshotSaveTime)) SaveSnapshot(P, Hosts);
	...
	if (!InterruptRun) RecordSample(t, P.NumSamples - 1, bmh, bmf, bmPixels, state, P, Hosts);
	fprintf(stderr, "\nEnd of run\n");
	t2 = t + P.SampleTime;

	while (fs)
	{
		fs = TreatSweep(t2, state, P, Hosts);
		t2 += P.SampleStep;
	}
	...
	if(!InterruptRun) RecordInfTypes(P, Hosts);
	return (InterruptRun);
}
...
//					fprintf(stderr, "@@## trigAlertC=%i P.PreControlClusterIdHolOffset=%lg \n",trigAlertC, P.PreControlClusterIdHolOffset);
	...
	{
		TSMean = TSMeanNE; TSVar = TSVarNE;
		CaptureBitmap	(bmPixels, state, P, Hosts);
